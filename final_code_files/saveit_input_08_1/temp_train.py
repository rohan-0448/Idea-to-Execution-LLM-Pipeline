...
# Correctly handle relative paths.  If training_data_fp is relative, make it absolute
if not os.path.isabs(training_data_fp):
    training_data_fp = os.path.join(os.getcwd(), training_data_fp)  # Join with current working directory
    
print(f"Attempting to read training data from: {training_data_fp}") # Print the path to help debug
try:
