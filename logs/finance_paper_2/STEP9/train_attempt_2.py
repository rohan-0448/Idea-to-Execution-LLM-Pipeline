...
# Print dataframe's head and column names
print(df.head())
print(df.columns)

# Check for missing values *before* dropping 'id'
print(df.isnull().sum())

# Now, drop 'id' if it exists, and drop NA values
if 'id' in df.columns:
...
# Double check that all the expected columns are available before proceeding.
expected_columns = ['score_10', 'next_day_direction', 'stock_id']
...
y = df['score_10'].values.tolist()  # Store labels
y = [int(2 * (score_10 + 1)) for score_10 in y]  # Convert labels from [-1, 1] to [0, 4] to convert into a classification problem

# Split data into training and validation sets
# Before splitting, convert stock_id to numeric
stock_ids = df['stock_id'].astype('category').cat.codes.values
X_train, X_val, y_train, y_val, stock_ids_train, stock_ids_val = train_test_split(X, y, stock_ids, test_size=0.2, random_state=42, stratify=y)
...
class NuminDataset(Dataset):
    def __init__(self, data, labels, stock_ids):
    ...
    def __getitem__(self, idx):
        ...
        stock_id = self.stock_ids[idx] # Get the stock ID
        return sample, label, stock_id  # Return sample, label, and stock_id

# Define MLP class
class MLP(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_stocks):
        super(MLP, self).__init__()
        self.embedding = nn.Embedding(num_stocks, 10) # Example embedding dimension of 10
        self.l1 = nn.Linear(input_size + 10, hidden_size)  # Input linear layer with dimensions input_size + embedding size to hidden_size
    ...
    def forward(self, X, stock_ids):
        embedded = self.embedding(stock_ids)
        X = torch.cat((X, embedded), dim=1)  # Concatenate the features and the embedding
...
mlp = MLP(INPUT_SIZE, HIDDEN_SIZE, OUTPUT_SIZE, num_stocks).to(DEVICE)  # Move model to the selected device

# Instantiate dataset and dataloader
train_dataset = NuminDataset(X_train, y_train, stock_ids_train)
val_dataset = NuminDataset(X_val, y_val, stock_ids_val)
...
for epoch in tqdm(range(NUM_EPOCHS)):  # Iterate through the dataset for NUM_EPOCHS
    ...
    train_loss = 0.0
    for i, (features, labels, stock_ids) in enumerate(train_dataloader):
    ...
    with torch.no_grad():  # Disable gradient calculation during validation
        for features, labels, stock_ids in val_dataloader:
            features = features.to(DEVICE)  # Move features to the correct device
            labels = labels.to(DEVICE)  # Move labels to the correct device
            stock_ids = stock_ids.to(DEVICE)
