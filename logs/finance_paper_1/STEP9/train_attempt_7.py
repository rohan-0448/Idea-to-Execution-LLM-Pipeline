...
required_keys = ['api_key', 'file_paths', 'training_data', 'output_directory']
for key in required_keys:
    if key not in config:
        logging.error(f"Missing key in config.yaml: {key}")
        raise ValueError(f"Missing key in config.yaml: {key}")

# Initialize Numin API object
try:
    napi = NuminAPI(api_key=config['api_key'])
    logging.info("Numin API initialized successfully.")
except Exception as e:
    logging.error(f"Failed to initialize Numin API: {e}")
    raise
...
try:
    ...
    logging.info(f"Data loaded successfully from {training_data_fp}")
except FileNotFoundError:
    logging.error(f"Training data file not found: {training_data_fp}")
    raise
except Exception as e:
...
for epoch in range(NUM_EPOCHS):
    ...
    logging.info(f"Epoch [{epoch+1}/{NUM_EPOCHS}], Training Loss: {avg_train_loss:.4f}, Validation Loss: {avg_val_loss:.4f}, Validation Accuracy: {accuracy:.2f}%")
    ...
    if avg_val_loss < best_val_loss:
        ...
        logging.info(f"Best model saved to {model_fp}")
