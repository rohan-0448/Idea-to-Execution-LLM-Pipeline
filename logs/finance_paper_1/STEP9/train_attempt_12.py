...
# Function to check config and raise value errors if they are not correct
def check_config(config):
    ...
    for key in required_keys:
        ...
        if key == 'file_paths':
            if not all(k in config['file_paths'] for k in ['training_data', 'output_directory']):
                raise ValueError("Missing 'training_data' or 'output_directory' in file_paths of config.yaml")
