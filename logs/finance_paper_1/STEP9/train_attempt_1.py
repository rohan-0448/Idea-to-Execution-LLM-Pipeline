...
try:
    ...
    logging.error(f"Error parsing config file: {e}. Exiting.")
    exit()
...
# file_path = config['file_paths']['training_data_zip']  # Path to save zip
...
try:
    ...
    logging.error(f"Training data file not found at {training_data_fp}. Exiting.")
    exit()
except Exception as e:
    logging.error(f"Error reading training data: {e}. Exiting.")
    exit()
...
HIDDEN_SIZE = config['model_params']['hidden_size']  # Size of hidden layers
OUTPUT_SIZE = NUM_ASSETS  # Output portfolio weights for each asset
NUM_EPOCHS = config['training_params']['num_epochs']
BATCH_SIZE = config['training_params']['batch_size']
LEARNING_RATE = config['training_params']['learning_rate']
VALIDATION_SPLIT = config['training_params']['validation_split']  # 80-20 split
...
def preprocess_data(df, list_of_ids):
    ...
    # Check if 'target' column exists before processing
    if 'target' in df.columns:
        # Target variable transformation
        df['target'] = [int(2 * (score_10 + 1)) for score_10 in df['target']]
    else:
        logging.warning("'target' column not found in DataFrame. Returning without transformation.")
        return df
...
if 'target' not in df.columns:
    logging.error("Target column missing after preprocessing. Check your data and preprocessing steps.")
    exit()
