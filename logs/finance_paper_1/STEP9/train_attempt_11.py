...
# Check if api_key is in the config
...
# df.dropna(inplace=True)  # Drop rows with missing values in the training data  #Move to preprocess
...
def preprocess_data(df, list_of_ids):
    ...
    df.fillna(0, inplace=True) # Handle missing values by imputation
...
train_dataset = PortfolioDataset(X[:train_size], y[:train_size])
val_dataset = PortfolioDataset(X[train_size:], y[train_size:])
