...
napi = NuminAPI(api_key=config['api_key']) # Get key from the config file
...
#Data Preprocessing
def preprocess_data(df, list_of_ids, constant_value): #Add constant_value as parameter
    # Filter IDs
...
constant_value = config['file_paths']['constant_value']
df = preprocess_data(df.copy(), list_of_ids, constant_value)
...
actor_criterion = nn.MSELoss()  # More appropriate for portfolio weights
...
for epoch in range(NUM_EPOCHS):
    ...
    for i, (features, labels) in enumerate(train_loader):
        ...
        actor_loss = actor_criterion(portfolio_weights, labels)  # Use MSELoss for portfolio weights
    ...
    with torch.no_grad():  # Disable gradient calculation for validation
        for features, labels in val_loader:
            ...
            portfolio_weights, value = model(features)
            actor_loss = actor_criterion(portfolio_weights, labels)  # Use MSELoss
