...
try:
    ...
    logging.error("config.yaml not found.  Make sure it exists in the correct location.")
    raise
...
try:
    ...
    logging.error(f"An error occurred while initializing NuminAPI: {e}")
    raise
...
try:
    ...
    logging.error(f"Training data file not found at {training_data_fp}")
    raise
except Exception as e:
    logging.error(f"An error occurred while reading the training data: {e}")
    raise
...
def preprocess_data(df, list_of_ids):
    # Filter IDs
    df = df[df['id'].isin(list_of_ids)].copy()
