...
# Check if 'api_key' exists in config and is not None
...
# Import data
training_data_dir = config['file_paths']['training_data_dir']  # Path to where the data is stored
...
for file in training_files:
    ...
    df = pd.concat([df, temp_df]) #Concatenate the files into a single dataframe
...
VALIDATION_SPLIT = config['training_parameters']['validation_split']  # 80-20 split
L2_REG = config['training_parameters']['l2_reg']
...
optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE, weight_decay=L2_REG) #Add L2 Regularization
