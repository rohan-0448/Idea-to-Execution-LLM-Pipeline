...
required_keys = ['file_paths', 'api_key']
...
assert all(id in df['id'].unique() for id in list_of_ids), "Not all ids in list_of_ids are present in the dataframe."
...
constant_value = 0.0  # Replace with an appropriate constant value
...
def time_based_split(df, validation_split=0.2):
    ...
    val_size = int(validation_split * len(df))
...
train_df, val_df = time_based_split(df.copy(), VALIDATION_SPLIT)

# Now create the datasets from the dataframes:
X_train = train_df.drop('target', axis=1).values.astype(np.float32)
y_train = train_df['target'].values.astype(np.int64)
X_val = val_df.drop('target', axis=1).values.astype(np.float32)
