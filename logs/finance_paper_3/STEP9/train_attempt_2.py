...
def main():
    ...
    feature_columns = config['features']['feature_columns'] #Load the features
    ...
    # 3. Create Datasets
    train_dataset, val_dataset = create_datasets(X, y, test_size=config['training_params']['test_size'], random_state=config['training_params']['random_state'])
    ...
    model = LSTM(input_dim=INPUT_SIZE, hidden_dim=config['model_params']['hidden_size'], layer_dim=config['model_params']['num_layers'], output_dim=len(np.unique(y))).to(DEVICE)
    ...
    # 7. Train the Model
    train_model(model, train_loader, val_loader, criterion, optimizer, config['training_params']['num_epochs'], DEVICE)
