...
def main():
    ...
    torch.save(imputer, os.path.join(output_directory, 'imputer.pth'))
    ...
    if testing_data_fp:
        ...
        test_df = acquire_testing_data(testing_data_fp)
        if test_df is None:
            print("Testing data not loaded, skipping testing.")
            return

        # 2. Load Scaler and Imputer
        scaler = torch.load(os.path.join(output_directory, 'scaler.pth'))
        imputer = torch.load(os.path.join(output_directory, 'imputer.pth'))

        # 3. Preprocess Testing Data
        X_test = preprocess_testing_data(test_df.copy(), feature_columns, scaler, imputer)
