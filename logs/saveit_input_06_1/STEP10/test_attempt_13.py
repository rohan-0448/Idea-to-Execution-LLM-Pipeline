...
class MCTD:
    ...
    def evaluate_trajectories(self, self, trajectories, market_state, expert_opinions):
...
class AutonomyOfExperts:
    def __init__(self, input_size, device, config):
        self.experts = {
        ...
        # Ensure the config file has the expert_models_dir
        expert_models_dir = config['file_paths'].get('expert_models_dir') #Use .get() to avoid KeyError

        if expert_models_dir is None:
            logging.error("Error: 'expert_models_dir' not found in config.yaml.")
            raise ValueError("Missing 'expert_models_dir' in config.yaml")

        # Construct the absolute path to the saved_models directory
        saved_models_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), expert_models_dir) # Corrected path
...
try:
    ...
    logging.error(f"MLP model not found: {e}")
    raise
...
try:
    ...
    logging.error(f"Scaler not found: {e}")
    raise
...
def run_inference(test_data, curr_round):
    ...
    for ticker in test_data['id'].unique():  # Iterate through all unique tickers
        ...
        if len(ticker_data.columns) < INPUT_SIZE:
            logging.warning(f"Ticker {ticker}: Input size less than expected. Padding with zeros.")
            ...
            logging.warning(f"Ticker {ticker}: Input size greater than expected. Truncating columns.")
        ...
        if not np.isnan(features).all():
            try:
                ...
                logging.error(f"Scaler transform error for ticker {ticker}: {e}")
                features = np.zeros((1, INPUT_SIZE))  #Use zero vector if scaling fails
        else:
            logging.warning(f"Ticker {ticker}: All features are NaN. Using a zero vector.")
...
while rounds_completed < NUM_ROUNDS:  # Run loop for all rounds
    try:
        ...
        logging.exception(f"An error occurred: {str(e)}")  # Log the exception with traceback
        time.sleep(WAIT_INTERVAL)
