...
def run_inference(test_data, curr_round):
    ...
    for ticker in test_data['id'].unique():  # Iterate through all unique tickers
        ...
        if len(ticker_data.columns) < INPUT_SIZE + 2: # Account for target_0 and target_10
            ...
            ticker_data = pd.concat([ticker_data, pd.DataFrame(padding)], axis=1)  # pad with zeros
            ticker_data = pd.concat([ticker_data, labels], axis=1)  # concatenate labels to the end
        elif len(ticker_data.columns) > INPUT_SIZE + 2:
