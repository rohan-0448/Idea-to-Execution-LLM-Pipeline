...
if not os.path.exists(training_data_fp):
    print(f"Error: The file '{training_data_fp}' does not exist.")
    print("Please ensure the file is in the correct directory and the path is correct.")

    # Option 1: Try to download the data (uncomment and adapt if needed)
    from numin import NuminAPI
    napi = NuminAPI(api_key= 'YOUR_API_KEY') # Replace with your actual API key
    try:
        data = napi.get_data(data_type="training")
        file_path = "training_data.zip"  # Change the file name as needed
        with open(file_path, 'wb') as f:
            f.write(data.getbuffer())
        print(f"Data downloaded and saved to {file_path}")
        # You'll likely need to unzip it and potentially rename the CSV file

        with zipfile.ZipFile(file_path, 'r') as zip_ref:
            zip_ref.extractall("./training_data")  # Extract to the expected directory
    except Exception as e:
        print(f"Error downloading or extracting data: {e}")
        exit()


    # Check if the file exists AFTER attempting download
    if not os.path.exists(training_data_fp):
        print("ERROR: Data file still not found after attempted download/extraction. Check download and extraction process.")
        exit() # Terminate the program if the file is still not found
    else:
        print("Data file found after download/extraction.")

    # Option 2: Exit the program gracefully
    # exit()
