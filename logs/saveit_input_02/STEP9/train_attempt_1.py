...
if not os.path.exists(training_data_fp):
    print(f"Error: File not found at {training_data_fp}")
    raise FileNotFoundError(f"The specified file does not exist: {training_data_fp}")
...
for epoch in tqdm(range(NUM_EPOCHS)): # iterate through the dataset for NUM_EPOCHS
    ...
    for i, (features, labels) in enumerate(train_dataloader):
        ...
        torch.nn.utils.clip_grad_norm_(lstm_dist.parameters(), max_norm=1) # Clip gradients at a maximum norm of 1
