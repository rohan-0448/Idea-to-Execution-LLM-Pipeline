...
# Option 1: Load the *best* model (recommended)
model_path = os.path.join(config['file_paths']['output_directory'], 'best_mlp.pth') # Construct the correct path
# Option 2: Load the *last* model
# model_path = os.path.join(config['file_paths']['output_directory'], 'last_mlp.pth')  # Construct the correct path

mlp = MLP(INPUT_SIZE, HIDDEN_SIZE, OUTPUT_SIZE) # instantiate mlp model with input size, hidden size and output size
mlp.load_state_dict(torch.load(model_path)) # load the model weights
