...
#mlp.to(DEVICE)  # Ensure the model is on the right device (GPU or CPU) - No longer needed, handled above.

# Construct file paths using os.path.join
output_dir = config['file_paths']['output_directory']
model_file_path = os.path.join(output_dir, 'best_mlp.pth')  # Or 'final_mlp.pth'
scaler_fp = os.path.join(output_dir, 'scaler.pkl')

# Before loading, check if both files exist
...
if not os.path.exists(scaler_fp):
    print(f"Scaler file not found at: {scaler_fp}")
    print(f"Contents of output directory {os.path.dirname(scaler_fp)}:")
    if os.path.exists(os.path.dirname(scaler_fp)):
        print(os.listdir(os.path.dirname(scaler_fp))) #Print the files present in the directory.
    else:
        print(f"Output directory {os.path.dirname(scaler_fp)} does not exist.")
...
try:
    ...
    logging.info(f"Scaler loaded successfully from {scaler_fp}")
except FileNotFoundError:
    logging.error(f"Scaler file not found at {scaler_fp}. Ensure training was completed and the path is correct.", exc_info=True)
    raise
...
def run_inference(test_data, curr_round):
    ...
    for ticker in test_data['id'].unique():  # Iterate through all unique tickers
        ...
        if len(ticker_data.columns) != INPUT_SIZE:
            raise ValueError(f"Input data has {len(ticker_data.columns)} features, but the model expects {INPUT_SIZE} features.")

        # Prepare data for inference
        features = ticker_data.iloc[-1, :].values  # Last timestep features, all columns

        if not isinstance(features, np.ndarray):
            features = np.array(features) #Convert to numpy array if not already.

        if not np.isfinite(features).all():
            raise ValueError("Input features contain non-finite values (NaN or inf).")
